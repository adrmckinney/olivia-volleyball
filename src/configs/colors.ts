...
export const colors = {
    ...
    bgNavBar: ({ hide, opacity }: { hide: boolean; opacity: string }) => {
        return [hide ? 'bg-transparent' : `bg-gray-900 ${opacity}`].join(' ');
    },
    textHistoryTitle: primaryText,
    textHistoryBody: primaryText,
    mobileNavMenuButton:
        'text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-purple-300',
